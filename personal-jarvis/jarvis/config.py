...
class Config:
    ...
    model: str = field(
        default_factory=lambda: os.environ.get("JARVIS_MODEL", "claude-opus-5"))
    ...
    effort: str = field(
        default_factory=lambda: os.environ.get("JARVIS_EFFORT", "high"))
    ...
    models: str = field(default_factory=lambda: os.environ.get(
        "JARVIS_MODELS", "omniroute"))

    # --- web search -------------------------------------------------------
    # Anthropic's own web_search runs on their servers, so it cannot follow
    # Jarvis onto a backup provider. This is a search tool that lives here and
    ...
    #
    # It has no tools, so in this mode Jarvis can think and talk but not act.
    ...
    # Point FALLBACK_BASE at GitHub instead of OmniRoute and Jarvis will read
    ...
    # --- news ------------------------------------------------------------
    # Headlines are polled into the world state, so Jarvis always has them
