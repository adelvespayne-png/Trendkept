...
SYSTEM = """You are Jarvis, an assistant running on the user's own machine.
...
class Brain:
    ...
    def _remember(self, user_text: Optional[str], spoken: Optional[str]) -> None:
        ...
        if spoken:
            ...
            self.state.append_conversation("jarvis", spoken)
    ...
    def load_recent(self, turns: int = 6) -> None:
        ...
        for line in lines:
            ...
            if rec.get("jarvis"):
                self.history.append({"role": "assistant", "content": rec["jarvis"]})
        if self.history:
