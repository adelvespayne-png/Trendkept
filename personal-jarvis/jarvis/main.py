"""Jarvis — the orchestrator.
    ...
    python -m jarvis.main               # full assistant, whatever hardware exists
    python -m jarvis.main --text        # typed input, spoken output
    python -m jarvis.main --say "..."   # one question, then exit
    python -m jarvis.main --check       # print what's on and what's off
...
consumer means Jarvis never talks over itself, and a burst of triggers during
...
class Jarvis:
    ...
    async def run(self) -> None:
        ...
        print(f"\nJarvis is up. Say “{self.cfg.wake_model.replace('_', ' ')}”. "
              "Ctrl-C to stop.\n")
        try:
            while self._running:
...
async def _one_shot(jarvis: Jarvis, text: str) -> int:
    jarvis._loop = asyncio.get_running_loop()
    jarvis._events = asyncio.Queue(maxsize=1)
    if not jarvis.brain.available:
        print("There is no ANTHROPIC_API_KEY set, so there is nothing to "
              "think with. Put one in .env — README step 5.")
        return 1
    reply = await jarvis.brain.respond(user_text=text, channel="text")
    if reply:
        print(reply)
        await asyncio.to_thread(jarvis.speaker.say, reply)
        return 0
...
def _check(cfg: Config) -> int:
    ...
    jarvis = Jarvis(cfg)
    rows = [
        ("microphone", jarvis.listener.mic.available,
         "pip install sounddevice, and grant mic permission"),
        ("wake word", jarvis.wake._ensure_model(),
         "pip install openwakeword"),
        ("speech out", jarvis.speaker.backend != "print",
         "install piper + a player (paplay/aplay/afplay)"),
        ("reasoning", jarvis.brain.available,
         "set ANTHROPIC_API_KEY in .env"),
        ("vision", jarvis.vision._open(),
         "set VISION_ENABLED=true, pip install opencv-python ultralytics"),
        ("home", jarvis.home.available,
         "set HA_ENABLED=true, HA_URL, HA_TOKEN"),
        ("phone", cfg.server_enabled and len(cfg.server_token) >= 16,
         "python -m jarvis.server --new-token, then SERVER_ENABLED=true"),
        ("news", jarvis.news.available, "set NEWS_ENABLED=true and NEWS_FEEDS"),
        ("health", jarvis.health.available,
    ...
    print(f"\n{len(rows) - missing}/{len(rows)} subsystems available. "
          "Jarvis runs with whatever it has.\n")
    return 0


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(prog="jarvis", description=__doc__.split("\n")[0])
    ...
    if args.say:
        return asyncio.run(_one_shot(jarvis, args.say))
    ...
    async def _go() -> None:
        ...
        task = asyncio.ensure_future(
            jarvis.run_text() if args.text else jarvis.run())
