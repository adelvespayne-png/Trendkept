"""Ambient awareness of the outside world — RSS headlines, polled.
    ...
    python -m jarvis.sensors.news       # fetch once and print what came back
...
interval and writes them into the world state, which means two things:
  ...
  * every answer Jarvis gives already has the day's news in context, without
...
UA = "jarvis-assistant/1.0 (personal use)"
...
def main(argv=None) -> int:
    ...
    print(f"\n{len(headlines)} headlines. These go into the world state, so "
          "Jarvis has them without searching.")
    return 0
