...
Jarvis ships set to run on free providers, so this comes first: OmniRoute
...
### 2. Unzip Jarvis somewhere sensible

`Documents\jarvis-assistant` is fine. Avoid OneDrive folders — the sync can
lock files while Jarvis is writing them.

### 3. Set it up

```powershell
cd $HOME\Documents\jarvis-assistant
python -m venv .venv
...
No player to install — Jarvis uses Windows' own audio.

### Speech in

```powershell
python -m jarvis.sensors.stt
```
...
```powershell
python -m jarvis.sensors.wake_word
```

Say "hey Jarvis". It prints a line each time it hears you. Then run the real
thing:
...
```powershell
New-NetFirewallRule -DisplayName "Jarvis" -Direction Inbound `
...
`python -m jarvis.main --check` lists every subsystem, on or off, with the
