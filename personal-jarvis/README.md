...
python -m jarvis.main --text          # type at it; no microphone needed
```
...
```bash
python -m jarvis.main --check         # what's on, what's off, and how to fix it
```
...
|---|---|
| `python -m jarvis.main` | The real thing: wake word, voice in, voice out |
| `python -m jarvis.main --text` | Type instead of speaking (sensors still run) |
| `python -m jarvis.main --say "..."` | One question, then exit |
| `python -m jarvis.main --check` | Subsystem availability report |
| `python -m jarvis.main --serve` | Also open the phone endpoint — see **PHONE.md** |
| `python -m jarvis.sensors.news` | Fetch the headlines once and print them |
| `python -m jarvis.harness` | Trigger rules against fake sensor data, no API key |

Each sensor module also runs standalone, which is how you debug one piece
without the rest: `python -m jarvis.sensors.stt`, `.tts "hello"`,
...
hardware, no network. `python -m jarvis.harness --repl` lets you type state
...
python -m jarvis.sensors.tts "Speech output is working."
python -m jarvis.sensors.stt      # records one sentence and prints it
```
...
**Manual setup: none** — openWakeWord downloads its own models on first run,
and `hey_jarvis` is one of the bundled ones. If it triggers on the
...
python -m jarvis.main --say "what do you know about the room?"
```
...
**When a model runs out.** `JARVIS_MODELS` is a chain, and the gateway sits
...
```
JARVIS_MODELS=omniroute,claude-opus-5
```
...
providers, switching itself when one runs dry. Jarvis just asks for the model
`auto` and lets it route. Anything OpenAI-compatible works here; point
`FALLBACK_BASE` at GitHub Models instead and `python -m jarvis.providers`
...
Jarvis can still read and write your map, set reminders, check the weather,
...
everything you say to Jarvis on that path and forwards it to whichever
...
Cost control is `JARVIS_EFFORT` (`low` → `max`, default `high`) and the fact
...
python -m jarvis.sensors.vision
```
...
mean Jarvis speaks; it means the brain is woken with the reason and can call
`stay_silent`, which it should do most of the time. `PROACTIVE_COOLDOWN`
(default 120s) is the floor between unprompted remarks.

Add your own rules to `DEFAULT_RULES` in `jarvis/core/triggers.py` — a
...
```bash
python -m jarvis.sensors.news        # fetch once and print the headlines
```

On by default. Headlines are polled every 15 minutes into the world state, so
they ride along with every answer — Jarvis knows roughly what today is about
...
```bash
python -m jarvis.server --new-token     # paste the line into .env
...
Jarvis can see this map on every question, through `map_read`, `map_add` and
...
python -m jarvis.sensors.home_assistant     # lists your entities
```
...
`CONTROLLABLE` in `jarvis/sensors/home_assistant.py` if you disagree.
...
Jarvis never talks over itself.
...
something's worth a look. At `JARVIS_EFFORT=medium` a typical exchange is a
