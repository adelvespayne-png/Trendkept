...
with disciplined risk**. Trendkept turns a mechanical trend-following ruleset into
...
Never chase a move that has already run far: Trendkept refuses any entry more than
...
python -m trendkept.cli backtest examples/sample_uptrend.csv --account 1000 --risk 0.02 -v

# The same on real AAPL data (bundled)
python -m trendkept.cli backtest examples/aapl_2015_2017.csv --account 1000 --risk 0.02 -v

# Ask the rules what to do *today* (the paper-trading helper)
python -m trendkept.cli scan examples/aapl_2015_2017.csv --account 1000 --risk 0.02
```
...
python -m trendkept.cli scan     --symbol MSFT --provider yahoo

# Save history to a CSV for offline/repeatable runs
python -m trendkept.cli fetch AAPL --out data/aapl.csv
python -m trendkept.cli backtest data/aapl.csv --account 1000
```
...
Trendkept talks to [Alpaca](https://alpaca.markets) for real-time-ish data and
...
python -m trendkept.cli manage

# Just one symbol, and actually amend/close orders on the paper account
python -m trendkept.cli manage AAPL --confirm
```
...
```cron
30 21 * * 1-5  cd /path/to/Trendkept && /usr/bin/python3 -m trendkept.cli manage --confirm >> manage.log 2>&1
```
...
| --- | --- |
| `trendkept/web.py` | Local browser dashboard (`python -m trendkept.web`) |
| `trendkept/data.py` | `Bar` type + robust CSV loader (column matching, adjustment) |
| `trendkept/indicators.py` | SMAs and **causal** swing-point detection |
| `trendkept/strategy.py` | The rules above, as signals (trend filter, entries, stop, exit) |
...
at the future looks brilliant and trades terribly; Trendkept refuses to peek.
...
## The business around Trendkept

This repo also contains a complete plan for turning Trendkept into an
owner-operated product business (**Trendkept Pro** — sell the discipline, not
