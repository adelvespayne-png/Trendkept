# Vesper on your phone

You say **"Hey Siri, Vesper"**, speak, and it answers out loud. Screen off,
hands free, from anywhere.

There are two Vesperes behind that one phrase, and it's worth knowing which
one you're talking to:

| | **Home Vesper** | **Pocket Vesper** |
|---|---|---|
...
pocket. What Pocket Vesper gives you is the voice and the thinking; what it
can't give you is the house.

If you want the full Vesper reachable at all times, the honest fix is
...
**1. Make a token.** In Terminal, in the `vesper-assistant` folder:
...
> Vesper refuses to open the endpoint at all if the token is missing or under
> 16 characters.
...
**3. Start Vesper with the door open:**
...
Open **Shortcuts** → **+** → name it exactly **Vesper** (that's the phrase
...
**7. Text** (after Otherwise)
   ...
   {"model":"claude-opus-5","max_tokens":1000,
    ...
    "system":"You are Vesper, a voice assistant. Everything you say is read aloud: one or two sentences of plain spoken English, no lists, no markdown. Lead with the answer. Dry and understated. If you don't know, say so in one sentence.",
...
iOS does not let any app register its own wake word. "Vesper" alone, from a
...
→ **Double Tap** → scroll down to the Shortcuts list → tap **Vesper**.
...
- **Lock Screen widget** — press and hold the lock screen → Customise → add
  a **Shortcuts** widget → pick Vesper. One tap, no unlock.
...
- **Just "Siri"** — on iOS 17 and later you can drop the "Hey": Settings →
  ...
  "Siri, Vesper". Two words rather than three.
...
> **Want to genuinely just say "Vesper"?** That works today at the laptop —
...
want Vesper reachable with the lid shut, that's the *Always-on* section.

## Reaching it from outside the house

The wifi-name check sends you to Pocket Vesper the moment you leave. To get
Home Vesper from anywhere:
...
3. In the Shortcut, change step 5's URL to that address. Leave everything
   ...
   you'll reach Home Vesper on mobile data too.
...
If what you actually want is full Vesper with the laptop shut in a drawer,
...
Either becomes the always-on Vesper; your daily laptop and your phone both
...
at your house anyway — so it'd be a more expensive Pocket Vesper, not a Home
one.
...
| Siri opens the app instead of running it | Rename the Shortcut to one clear word: **Vesper** |
...
To watch what the laptop sees, run it with `python -m vesper.main --serve
