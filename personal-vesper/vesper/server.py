"""The phone bridge — a small HTTP endpoint on the laptop.
    ...
    python -m vesper.server --new-token     # make a token for .env
    python -m vesper.main --serve           # assistant + endpoint together
...
LOG = logging.getLogger("vesper.server")
...
class AskServer:
    ...
    def __init__(self, vesper: Any, loop: asyncio.AbstractEventLoop,
    ...
    def ask(self, text: str) -> str:
        ...
        if reply and self.cfg.server_speak_aloud:
            ...
            threading.Thread(target=self.vesper.speaker.say, args=(reply,),
                             daemon=True).start()
    ...
    def start(self) -> bool:
        ...
        if not token or len(token) < 16:
            LOG.error("SERVER_TOKEN is missing or too short; the phone bridge "
                      "will not start. Run: python -m vesper.server --new-token")
            return False
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _get(self) -> None:
            ...
            if path == "/health":
                if not self._check_token():
                    return
                self._send(200, {"ok": True, "vesper": "awake"})
            elif path == "/map":
                if not self._check_token():
                    return
                ...
                if not self._check_token():
                    return
                store = getattr(bridge.vesper, "map", None)
                if store is None:
                ...
                self._send(200, {
                    "vesper": "awake",
                    "world": bridge.vesper.state.snapshot().describe(),
        ...
        def _post(self) -> None:
            ...
            if path == "/health":
                ...
                feed = getattr(bridge.vesper, "health", None)
...
def main(argv=None) -> int:
    ...
    if args.new_token:
        ...
        print("\nAlso set SERVER_ENABLED=true, then start Vesper with:")
        print("  python -m vesper.main --serve\n")
        return 0

    print("The bridge runs as part of the assistant, not on its own:")
    print("  python -m vesper.main --serve")
    print("\nTo create a token:  python -m vesper.server --new-token")
